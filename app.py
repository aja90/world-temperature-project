...
df_season = pd.read_csv('seasonal_data.csv',index_col=0)
# sidebar
...
# Sidebar filters
    ...
    sns.lineplot(x='Year', y='Value', data=asia_df,  color='blue',errorbar=None)
    sns.lineplot(x='Year', y='Value', data=africa_df,  color='red',errorbar=None)
    sns.lineplot(x='Year', y='Value', data=europe_df,  color='green',errorbar=None)
    sns.lineplot(x='Year', y='Value', data=north_american_df,  color='orange' , errorbar=None)
    sns.lineplot(x='Year', y='Value', data=south_american_df,  color='purple' , errorbar=None)
    ...
    df = df.rename(columns = {'Population growth (annual %)':'Population',\
           'Annual methane emissions in CO₂ equivalents':'methane emissions',\
          'Annual nitrous oxide emissions in CO₂ equivalents':'nitrous oxide',\
            'Annual CO₂ emissions growth (%)': 'CO2 emissions', 'GDP (in USD)': 'GDP'})
    ...
    plt.title("Total CO2 Emissions by Country (Aggregated Over 25 Years)")
    st.pyplot(plt)
    ...
    # train test
    st.write(models_metrics.loc[:,models_metrics.columns.str.endswith('Train')])
    # test set
    st.write('Models Performance on test set:')
    st.write(models_metrics.loc[:,models_metrics.columns.str.endswith('Test')])
